...
-- Filter out tuples not between June 2007 and May 2008 on both F1 and F2
FilteredPairsByDate = FILTER FilteredPairsByTime BY (((F1::year == 2007 AND F1::month >= 6) OR (F1::year == 2008 AND F1::month <= 5)) AND ((F2::year == 2007 AND F2::month >= 6) OR (F2::year == 2008 AND F2::month <= 5)));
...
STORE AverageDelay INTO 's3://zycao-cs6240/output/pig1_1' USING PigStorage(',');
